...
本檔補強 `.claude/skills/self-audit/SKILL.md` 的通用 12 條 hard check（Layer 1），加上**期權研究專屬**的 silent-bug 守門員（公式 + 邊界 reference，非 procedural）。

用法：見 `CLAUDE.md` §2「Self-Audit SOP（三分級觸發）」。
